...
void dns_a2(int argc, char* argv[]) 
{
    if(argc != 3){
    ...
    struct RES_RECORD_OLD* answer=(struct RES_RECORD_OLD*)(data+length+sizeof(struct dataEnd));
    // printf("Data pointer base : %u\n", (int) data);
    // printf("Data struct pointer base : %u\n", (int) end);
    // printf("RES Record struct pointer base : %u\n", (int)answer);

    strcpy(answer, "\4test\3com");
    answer->type = htons(1);
...
void dns_a(char *src_ip, char *dst_ip, char *query, char *query_answer, char *dst_buffer, unsigned short *dst_pcktLen) 
{
    ...
    //refer to a query
    strncpy(data+query_len+sizeof(struct dataEnd), query, query_len);

    struct RES_RECORD *answer=(struct RES_RECORD*)(data+query_len+sizeof(struct dataEnd)+query_len);
    // printf("Data pointer base : %u\n", (int) data);
    // printf("Data struct pointer base : %u\n", (int) end);
    // printf("RES Record struct pointer base : %u\n", (int)answer);
    ...
    unsigned short int packetLength =(sizeof(struct ipheader) + sizeof(struct udpheader)+sizeof(struct dnsheader)+query_len+sizeof(struct dataEnd)+sizeof(struct RES_RECORD)+query_len); // length + dataEnd_size == UDP_payload_size
    //printf("Packet length = %d\n", packetLength);
    ip->iph_len = htons(packetLength);
    ip->iph_ident = htons(rand()); // we give a random number for the identification#
    ip->iph_ttl = 110; // hops
    ...
    // Destination port number
    udp->udph_destport = htons(53);
    udp->udph_len = htons(sizeof(struct udpheader)+sizeof(struct dnsheader)+query_len+sizeof(struct dataEnd)+sizeof(struct RES_RECORD)); // udp_header_size + udp_payload_size
    ip->iph_chksum = csum((unsigned short *)buffer, sizeof(struct ipheader) + sizeof(struct udpheader));
    udp->udph_chksum=check_udp_sum(buffer, packetLength-sizeof(struct ipheader));
    udp->udph_chksum=check_udp_sum(buffer, packetLength-sizeof(struct ipheader)); // recalculate the checksum for the UDP packet
